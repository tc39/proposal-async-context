...
async def main():
  ...
  print("after gater:", current_task.get())
...
This is the default `asyncio` scheduling behavior. Python allows userland scheduler to
customize the  propagation behavior around `await` with `context.copy` and `context.run`. So, if
a user run `context.run` without `asyncio` on an awaitable object, it can achieve
